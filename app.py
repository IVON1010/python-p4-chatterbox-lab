...
def messages():
    if request.method == "GET":
        ...
        if not body or not username:
            return make_response(jsonify({"error": "Both body and username should be provided"}), 400)
...
def message_by_id(id):
    ...
    if request.method == "GET":
        ...
        if not new_body:
            return make_response(jsonify({"error": "The 'body' parameter is required to update the message"}), 400)
